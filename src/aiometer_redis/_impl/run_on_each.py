...
async def run_on_each(
    ...
    *,
    ...
    if max_at_once is not None:
        meters.append(HardLimitMeter(max_at_once, redis_url=redis_url, key_prefix=key_prefix))
    if max_per_second is not None:
        meters.append(RateLimitMeter(max_per_second, redis_url=redis_url, key_prefix=key_prefix))
